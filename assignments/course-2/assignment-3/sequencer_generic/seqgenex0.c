...
#define ASSIGNMENT 3   // assignment number

#define ABS_DELAY  // Flag for absolute delay
#define DRIFT_CONTROL // Flag for drift control
#define NUM_THREADS (4+1) // Number of threads

int abortTest=FALSE; // When true, aborts Service
int abortS1=FALSE, abortS2=FALSE, abortS3=FALSE, abortS4=FALSE; // When true, aborts Service
sem_t semS1, semS2, semS3, semS4; // Service semaphores
...
void main(void)
{
    ...
    // Service_4 = Period 20
    rt_param[1].sched_priority = rt_max_prio - 4; // Set priority for Service_4
    pthread_attr_setschedparam(&rt_sched_attr[4], &rt_param[4]); // Set scheduling parameters for thread 4
    rc = pthread_create(&threads[4], &rt_sched_attr[4], Service_4, (void *)&(threadParams[4])); // Create thread for Service_4
    if (rc < 0) perror("pthread_create for service 4"); // Check for thread creation errors for Service_4
    else printf("pthread_create successful for service 4\n"); // Print success message if thread creation for Service_4 is successful
...
void *Sequencer(void *threadp) {
    ...
    do {
        ...
        if ((seqCnt % 20) == 0) {
            // Construct a message string for syslog with thread-specific information
            sprintf(msg, "Thread 4 start %d @ %lf on core %d \n",
                    seqCnt + 1,
                    current_time,
                    sched_getcpu());
            // Log the message to syslog with specified course and assignment identifiers
            log_sys(msg, COURSE, ASSIGNMENT);
            // Post the semaphore for Service_4
            sem_post(&semS3);
        }
...
void *Service_4(void *threadp)
{
    ...
    // Initialize a counter for Service 4
    ...
    while (!abortS4)
    {
        ...
        // Increment the counter for Service 4
        S4Cnt++;
