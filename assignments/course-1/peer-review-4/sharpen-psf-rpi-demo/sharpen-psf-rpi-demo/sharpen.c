...
int main(int argc, char *argv[])
{
    ...
    for (loop_count=1; loop_count <= 10; loop_count++) {
        ...
        printf("Iteration: %d, start test at %lf ", loop_count, fnow-fstart);
        real_start = fnow-fstart;
    ...
    clock_gettime(CLOCK_MONOTONIC, &now);
    fnow = (FLOAT)now.tv_sec  + (FLOAT)now.tv_nsec / 1000000000.0;
    real_stop = fnow-fstart;
    printf("stop test at %lf for %d frames taking %lf seconds \n", fnow-fstart, ITERATIONS, real_stop-real_start);
    }
